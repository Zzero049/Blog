...
**不要用递归，n如果很大(Integer.MIN_VALUE)，会栈溢出**
...
class Solution {
    public double myPow(double x, int n) {
        long N = n;							// 这里取long 是因为 n要是Integer.MIN_VALUE取负还是自己
        return N > 0 ? getTheResult(x, N) : 1.0 / getTheResult(x, -N);
    }

    private static double getTheResult(double x, long n) {
        double ans = 1.0;
